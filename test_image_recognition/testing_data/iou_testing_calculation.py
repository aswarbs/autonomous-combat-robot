...
def get_iou_array(ground_truth_array, pred_array, frame_len, path):
     ...
     with os.scandir(path) as it:
          ...
          for entry in it:
               ...
               cnter += len(ground_truth)
               diff = len(ground_truth) - len(pred)
               true_negs += diff
...
ground_truth_path = r"test_image_recognition/testing_data/qr/real_world/instances_default.json"
ground_truth_data = get_data_from_coco(ground_truth_path, frame_len)

ious, negs = get_iou_array(ground_truth_data, preds, frame_len, path)

print(f"mean: {np.mean(ious)}")
print(f"std dev: {np.std(ious)}")
print(f"true negatives: {negs}")
